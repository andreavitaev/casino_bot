...
def on_spin_pull(call: CallbackQuery):
    ...
    def run_spin():
        try:
            ...
            active = shop_get_active_for_game(uid, game_id)  # ТОЛЬКО активки, применяемые к этой игре
            boosts_line = render_active_boosts(dict(active))
            ...
            # Страховка: отменяем проигрыш, но продаём жизнь демону
            if insured and raw_delta < 0:
