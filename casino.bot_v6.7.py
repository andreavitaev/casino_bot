...
BOT_TOKEN = "8524213683:AAHO9OJ4sFVzcEJWJEQHm6tUq99psWE0suU"
...
def render_active_boosts(active: dict) -> str:
    ...
    Текущие усиления:
    🍷 Вино 🧲 Магнит
    Если нечего показывать — возвращает пустую строку.
    """
    if not active:
        return ""
    ...
    for k, v in active.items():
        ...
        title = str(item.get("title") or k).strip()
        if not title:
            continue
    ...
    # ВАЖНО: никогда не возвращаем None — только строку
...
def on_mail_open(call: CallbackQuery):
    ...
    db_exec(
        "UPDATE daily_mail SET pending_amt_cents=0, pending_msg_id=0, pending_kind='' WHERE user_id=?",
        (uid,),
        commit=True
    )
    ...
    def run_spin():
        try:
            ...
            if insured and int(delta) < 0:
                raw_loss = abs(int(delta))
                set_slave_buyout(uid, raw_loss * 100)
                ...
                delta = 0
                db_exec(
                    "DELETE FROM shop_active WHERE user_id=? AND item_key='insurance'",
                    (int(uid),),
                    commit=True
                )
            
            # Пересобираем активки для отображения ПОСЛЕ возможного списания страховки
            active_for_display = shop_get_active_for_game(uid, game_id)
            boosts_line = render_active_boosts(active_for_display)
            boosts_block = (boosts_line + "\n\n") if boosts_line else ""
